  ...
  it('erzeugt über 1.000 Seeds je Stufe systematisch zählbare Kombinationen', () => {
    ...
    for (const difficulty of [1, 2, 3] as const) {
      ...
      for (let seed = 1; seed <= 1_000; seed += 1) {
        ...
        const pairing = exercise.steps?.find((step) => step.id === 'missingPair')
        ...
        expect(new Set(pairing?.options?.map((option) => option.value)).size).toBe(3)
        ...
        expect(exercise.representation?.valueRoles.unknownValues).toEqual(expect.arrayContaining(['missingPair', 'combinationCount']))
        expect(exercise.representation?.valueRoles.revealedValues).toEqual([])
