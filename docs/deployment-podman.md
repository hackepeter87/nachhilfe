...
Ein Git-Tag wie `v0.7.0` erzeugt die Image-Tags `0.7.0`, `sha-<kurzsha>` und `latest`. Eine manuelle Ausführung über `workflow_dispatch` erzeugt nur das nachvollziehbare SHA-Tag. Normale Pushes auf `main` veröffentlichen kein Image.
