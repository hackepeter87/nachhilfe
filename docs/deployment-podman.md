...
Ein Git-Tag wie `v0.9.1` erzeugt die Image-Tags `0.9.1`, `sha-<kurzsha>` und `latest`. Eine manuelle Ausführung über `workflow_dispatch` erzeugt nur das nachvollziehbare SHA-Tag. Normale Pushes auf `main` veröffentlichen kein Image.
