...
Nach Deployment ist die URL des konkreten Testsystems hier einzutragen. Lokal startet die App mit npm run dev; der AMD64-Container mit `docker run --rm -p 8080:8080 mathe-reise:0.32.1`. Das öffentliche OCI-Image ist `ghcr.io/hackepeter87/nachhilfe:0.32.1`, sobald der Release-Workflow für `v0.32.1` erfolgreich gelaufen ist. Dieses Paket bereitet die Gesamtprüfung vor; eine tatsächliche Prüfung ist noch nicht dokumentiert.
