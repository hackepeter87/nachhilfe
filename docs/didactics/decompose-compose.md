# Zahlen zerlegen und zusammensetzen

## 1. Bezug zum Förderbedarf
Die additive Struktur dreistelliger Zahlen bildet die Grundlage für flexible Rechenstrategien.
## 2. Lehrplanbereich
Zahlen und Operationen; Darstellen und Argumentieren.
## 3. Konkretes Lernziel
Zahlen bis 1000 sicher zwischen Zahl, Stellenwertsumme und Materialdarstellung wechseln.
## 4. Voraussetzungen
Hunderter, Zehner und Einer unterscheiden.
## 5. Typische Fehlvorstellungen
Ziffern ohne Stellenwert addieren; Stellen vertauschen; Null als Platzhalter auslassen.
## 6. Fachliche Kernidee
Jede Zahl ist eindeutig die Summe ihrer Hunderter-, Zehner- und Einerwerte.
## 7. Methodischer Zugang
Material sortieren, Stellenwerttafel füllen, Summe schreiben und den Weg umkehren.
## 8. Geeignete Darstellungen
Stellenwerttafel und gebündeltes Material.
## 9. Lernprogression
Alle Stellen besetzt; eine Null; mehrere Nullstellen und reduzierte Hilfe.
## 10. Aufgabentypen
Zerlegung auswählen und Zahl aus vorgegebenen Stellenwerten eingeben.
## 11. Hilfestufen
Stellen markieren; Werte statt Ziffern notieren; Summe stellenweise zusammensetzen.
## 12. Remediation
Jede Ziffer zunächst sichtbar in die passende Spalte legen.
## 13. Transfer
Zwei verschiedene Zerlegungen derselben Zahl vergleichen, beispielsweise `300 + 40` und `340`.
## 14. Wiederholung
Nullmuster und Darstellungswechsel werden zeitversetzt variiert.
## 15. Erfolgskriterien
Zerlegung ergibt exakt die Ausgangszahl; Zusammensetzung bleibt im Zahlenraum.
## 16. Grenzen der aktuellen Umsetzung
Nichtkanonische Zerlegungen wie `2H + 14Z` sind noch nicht aktiv.
## 17. Punkte für die Gesamtprüfung
Prüfen, wann nichtkanonische Zerlegungen für den konkreten Förderstand sinnvoll werden.
